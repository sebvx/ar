...
#dbus-broker
pacman -S --noconfirm dbus-broker                         # Уставновка
...
# проги доп
pacman -Sy --noconfirm bat docker docker-compose gping tldr fd glances
# KDE
pacman -S --noconfirm xorg sddm plasma-desktop plasma-wayland-session breeze-gtk kde-gtk-config
pacman -Rsn --noconfirm kwayland-integration kwallet-pam plasma-thunderbolt plasma-vault powerdevil plasma-sdk kgamma5 drkonqi discover oxygen bluedevil plasma-browser-integration plasma-firewall
sudo pacman -Rsn --noconfirm plasma-pa     # Удаляем виджет управления звуком.
sudo pacman -S --noconfirm kmix            # Замена виджету plasma-pa, совместим с ALSA.
